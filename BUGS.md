...
### CR-6. Нет проверенной стратегии бэкапов БД (disaster recovery)
- **Подтверждено владельцем 2026-05-03:** Supabase managed-бэкапы скорее всего НЕ настроены (план Supabase нужно проверить в Dashboard). Восстановление из бэкапа никогда не тестировалось.
- **Важное различие:**
  - Таблица `Tvoy_vector_2_backups` (используется в `server/storage.ts:1899-1952`, активно создаётся при каждом логине, см. `server/routes.ts:340`) — это **in-app фича**: JSON-снапшоты данных одного репетитора, которые он может скачать сам. **Это НЕ disaster recovery** — лежит в той же Supabase, при потере БД пропадут вместе со всем остальным.
  - Реальных бэкапов всей БД (внешний `pg_dump` в S3/Backblaze, или Supabase PITR) — **нет или не подтверждено**.
- **Проблема:** при инциденте на стороне Supabase (incident, drop tables, неправильная миграция) — потенциальная потеря всех данных без возможности восстановления. Это полностью убивает бизнес.
- **Что делать:** срочно настроить ежедневный `pg_dump` на внешний storage (Backblaze B2 / S3) + протестировать восстановление в тестовую БД. План — в отдельном документе (см. ниже в разговоре).
...
### W-13. `promoCode` в покупке AI-пакета может игнорироваться сервером
- **Где:** `client/src/pages/subscription.tsx:686` (после фикса типа в W-10a — `promoCode` добавлен в payload).
- **Проблема:** до фикса 2026-05-03 тип мутации был `{ credits; pricePaid }` без `promoCode`, но вызов передавал три аргумента. TypeScript молча отбрасывал лишнее поле через структурную совместимость. Теперь поле в payload, но **неясно, обрабатывает ли его эндпоинт `/api/ai-packages/purchase`**.
- **Симптомы (если воспроизведутся):** скидка по промокоду в диалоге показывается, но в ЮKassa списывается полная сумма; либо `usedCount` промокода не инкрементируется.
- **Что делать:**
  1. Купить AI-пакет с тестовым промокодом, посмотреть в ЮKassa-логах сумму платежа.
  2. После успеха проверить в БД: `SELECT used_count FROM "Tvoy_vector_2_promo_codes" WHERE code='TESTCODE'`.
  3. Если игнорируется — найти обработчик `/api/ai-packages/purchase` в `server/routes.ts` и добавить применение скидки + запись в `Tvoy_vector_2_promo_code_redemptions`.
...
- **Что делать:** не баг, но странно. Возможно, задачи трекаются вне репо. Уточнить, где (Notion / Telegram / в голове).
