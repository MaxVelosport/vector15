...
- **2026-05-03 (вечер)** — W-12 закрыт целиком. Realtime-коллаборация на доске Excalidraw восстановлена: 6 связанных багов исправлены (prop удалён в 0.18 — теперь `updateScene`; все курсоры под одним ключом — теперь `socketId` per connection; репетитор слал чужое имя — теперь `useAuth`; ученик не слал курсор вообще — добавлен `onPointerUpdate`; сервер не шлёт `socketId` в broadcast — теперь шлёт; цвета по роли через `ROLE_COLORS`). Smoke-test прошёл. Этап раннего тестирования ROADMAP можно начинать с этой фичи.
